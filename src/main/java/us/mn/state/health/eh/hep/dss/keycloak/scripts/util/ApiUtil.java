...
public class ApiUtil {
    ...
    public static void setClientSecret(RealmResource realm, String clientId, String secret){
        ClientResource clientResource = findClientByClientId(realm, clientId);
    ...
    public static GroupResource findGroupByName(RealmResource realm, String groupName) {
        for (GroupRepresentation c : realm.groups().groups()) {
            if (groupName.equals(c.getName())) {
                return realm.groups().group(c.getId());
            }
        }
        return null;
    }
